...
$widget.nextId('popupmenu');$widget.buttonFlyout({focusManagement:true});$rootMenu.commonKeyDown();$rootMenu.rovingTabindex('[role^=menuitem]',{axis:'y'});$rootMenu.prop('id',$widget.prop('id')+'-menu');$button.attr('aria-haspopup','true').prop('id',$widget.prop('id')+'-button');$subMenus.attr('aria-expanded','false');$rootMenu.on('click spaceKeyDown enterKeyDown',function(e){var $menuitem=$(e.originalEvent.target);var role=$menuitem.attr('role');switch(role){case"menuitemradio":$radios.attr('aria-checked','false');$menuitem.attr('aria-checked','true');break;case"menuitemcheckbox":$menuitem.attr('aria-checked',$menuitem.attr('aria-checked')==='true'?'false':'true');break;default:break;}
$widget.trigger('menuSelect');});$widget.on('buttonFlyoutOpen buttonFlyoutClose',function onShowOrHide(){$allMenuItems.attr('tabindex','-1');});if(isSupportShortcutKey){$allMenuItems.on('keydown',function(e){var char=keyCodeMap[e.keyCode];var itemIndex=shortcutKeyMap[char];if(itemIndex){$allMenuItems.get(itemIndex).focus();}});}
